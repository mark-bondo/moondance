        ...
        sql = """
            WITH recipe AS (
                ...
                SELECT
                    ...
                    SUM(recipe.quantity * weight.conversion_rate * child.unit_material_cost)::NUMERIC(16, 6) as material_cost,
                    SUM(recipe.quantity * weight.conversion_rate * child.unit_material_cost * (pcode.freight_factor_percentage/100::NUMERIC))::NUMERIC(16, 6) as freight_cost,
                    SUM(
                    ...
                    SUM(
                            COALESCE(inventory.quantity_onhand * weight_inventory.conversion_rate, child.total_quantity_onhand * weight_child.conversion_rate, 0) 
                    )::NUMERIC(16, 2) as quantity_onhand
                FROM
                    ...
                    LEFT JOIN public.operations_inventory_onhand inventory ON child.id = inventory.sku_id 
                    ...
                    JOIN public.operations_weight_conversions weight_child ON
                        child.unit_of_measure = weight_child.from_measure AND
                        recipe.unit_of_measure = weight_child.to_measure
                WHERE
            ...
            FROM
                (
                SELECT 
                    (select row_to_json(_) from (select product_id, wip, product_description, family, sku, description, unit_of_measure, quantity_needed::NUMERIC(16, 5), total_cost::NUMERIC(16, 5), quantity_onhand::NUMERIC(16, 5)) as _) as json
                FROM
                    recipe
                WHERE
                    product_id = ANY(%s)
