...
class Amazon_API(object):
        ...
        if command == "sales_order_lines":
            ...
            for i, o in enumerate(order_lines):
                try:
                    ...
                    if error_count <= 1:
                        self.logger.warning(
                            "sync amazon {}: failed getting order lines, getting new refresh token".format(
                                self.command
                            ),
                            exc_info=1,
                        )
                        ...
                        self.logger.error(
                            "sync amazon {}: failed getting order lines {} times, exiting program".format(
                                self.command, error_count
                            ),
                            exc_info=1,
                        )
                        break
    ...
    def sync_data(self):
        ...
        self.logger.info(
            'sync amazon {}: inserting {} rows into "{}"'.format(
    ...
    def build_parameters_string(self):
        ...
        self.object_dd["request_parameters"] = collections.OrderedDict(
            sorted(self.object_dd["request_parameters"].items())
        )
        self.request_parameter_string = "&".join(
            [
                "{}={}".format(urllib.parse.quote_plus(k), urllib.parse.quote_plus(v))
    ...
    def get_data(self):
        ...
        with open(self.object_dd["file_name"], "w") as w:
            ...
            self.logger.info(
                'sync amazon {}: written {} rows to file "{}"'.format(
